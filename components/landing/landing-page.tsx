      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-[60] w-full">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <h1 className="text-xl md:text-2xl font-bold text-gray-900">FoodWallah</h1>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            {isLoggedIn && user ? (
              <div className="flex items-center gap-2">
                <span className="hidden md:inline text-sm font-medium text-gray-700">Hi, {user.firstName}</span>
                <button onClick={logout} className="text-gray-600 hover:text-red-500"><LogOut className="w-4 h-4" /></button>
              </div>
            ) : (
              <button className="text-gray-900 hover:text-orange-500 text-sm font-bold md:font-medium">SignIn</button>
            )}
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-12 overflow-hidden">
          <div className="flex items-center justify-between mb-8">
            <p className="text-lg md:text-2xl font-bold text-gray-900">Hey! What's on your mind</p>
            <div className="flex items-center gap-2">
               <button onClick={() => scroll("left")} className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center text-white"><ChevronLeft className="w-5 h-5" /></button>
               <button onClick={() => scroll("right")} className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-500"><ChevronRight className="w-5 h-5" /></button>
            </div>
          </div>
            ...
            <div className="grid grid-rows-2 grid-flow-col gap-x-6 gap-y-6 md:flex md:flex-row md:gap-8 w-max md:w-full">
              {categories.map((category, index) => (
                ...
                >
                  ...
                  <span className="text-[10px] md:text-sm font-bold md:font-medium text-gray-700">{category.name}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-2 no-scrollbar whitespace-nowrap">
          ...
          <button className="px-4 py-2 border border-gray-200 rounded-full text-xs font-medium bg-white shadow-sm">Restaurant Type ▼</button>
          <button className="px-4 py-2 border border-gray-200 rounded-full text-xs font-medium bg-white shadow-sm flex items-center gap-1">Sort ≡</button>
        </div>
        ...
        <div className="text-center py-8">
          <p className="text-gray-500 text-xs md:text-sm animate-pulse">Loading More Restaurants.....</p>
        </div>
      </main>
